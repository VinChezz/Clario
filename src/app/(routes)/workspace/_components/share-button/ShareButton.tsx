...
export default function ShareButton({ fileId, fileName, children }: ShareButtonProps) {
    ...
    const handleShareToogle = async (isPublic: boolean, permissions = "VIEW") => {
      ...
      toast.success(
        isPublic ? 'Access is open' : 'Access denied', 
        {
    ...
    const shareToTelegram = () => {
      ...
      const telegramUrl = `https://t.me/share/url?url=${encodeURIComponent(getShareableLink())}&text=${encodeURIComponent(text)}`;
    ...
    return (
      <Dialog>
        <DialogTrigger asChild>
          <Button className='h-8 text-[12px] gap-2 bg-blue-600 hover:bg-blue-700' onClick={fetchShareInfo} >
        ...
        <DialogContent className='sm:max-w-md'>
          <DialogHeader>
            ...
            <DialogDescription>Set up access to file "{fileName}"</DialogDescription>
          </DialogHeader>
